	...
	var NEWS = {
			...
			if (_owlContainer.length) {
				_owlContainer.each(function () {
					...
					_owlContainer.owlCarousel({
						...
						navText: ['<i className="feather icon-chevron-left"></i>', '<i className="feather icon-chevron-right"></i>']
					});
		...
		slickSync: function () {
			...
			if (_slickCont.length) {
				...
				var _prevArrow = '<button type="button" className="slick-prev"><i className="feather icon-chevron-left"></i></button>',
					_nextArrow = '<button type="button" className="slick-next"><i className="feather icon-chevron-right"></i></button>';
		...
		magnificPopup: function () {
			if (NEWS.videoPopup.length) {
				NEWS.videoPopup.magnificPopup({
					...
					mainClass: 'mfp-fade',
					iframe: {
						markup: '<div className="mfp-iframe-scaler">' + '<div className="mfp-close"></div>' + '<iframe className="mfp-iframe" frameborder="0" allowfullscreen></iframe>' + '</div>',
						patterns: {
							youtube: {
								index: 'youtube.com/',
								id: 'v=',
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (data.code == false) {
							...
							_self.find('.btn-primary').after('<div className="error-msg"><p>*' + data.err + '</p></div>');
						} else {
							$('.error-msg').hide();
							$('.form-group').removeClass('focused');
							_self.find('.btn-primary').after('<div className="success-msg"><p>' + data.success + '</p></div>');
